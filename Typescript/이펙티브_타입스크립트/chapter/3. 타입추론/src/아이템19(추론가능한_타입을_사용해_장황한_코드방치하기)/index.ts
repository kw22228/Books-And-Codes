...
(function () {
    ...
    function logProduct(product: Product) {
        const id: number = product.id; //타입선언으로 인해 추론이 되어지지않아 오류가 나고있음.
    ...
    function logProduct2(product: Product) {
        ...
        const { id, name, price }: { id: string; name: string; price: number } = product; //아주 번잡한 코드가 되어버림.
        console.log(id, name, price);
    }

    /** 매개변수에 타입구문을 생략하는 경우 */
    function parseNumber(str: string, base = 10) {} //base 매개변수의 default값이 10(number)이기 떄문에 알아서 추론됨.
    ...
    logProduct2(furby);
    const furbyWithType: Product = {
        name: 'Furby',
        id: 123121353, //string이여야함
        price: 35,
    };
    ...
    function getQuoteWithoutReturnType(ticker: string) {
        if (ticker in cache) {
    ...
    function getQuoteWithReturnType(ticker: string): Promise<number> {
        if (ticker in cache) {
